...
def load_config_strict(config_path):
    """
    Load ZacQC trading parameters from a `parameters.py` file.

    Parameters
    ----------
    config_path : str
        Absolute path to the `parameters.py` file.
...
def save_config_to_parameters_py(config_dict, config_path):
    """
    Persist configuration values back into `parameters.py`.

    Parameters
    ----------
    config_dict : dict
        Updated configuration values keyed by attribute name.
    config_path : str
        Absolute path to the `parameters.py` file.
    ...
    -------
    bool
        True when the file was successfully written, otherwise False.
    """
    import re
...
def setup_logging():
    """
    Configure rotating file and console logging for the server.

    Returns
    -------
    str
        Absolute path to the directory that stores server logs.
    """
...
def cleanup_old_backtests(backtests_dir, keep_latest=5, max_total_size_mb=500):
    """
    Delete older backtest folders when retention or size limits are exceeded.

    Parameters
    ----------
    backtests_dir : str
        Directory that stores timestamped backtest outputs.
    keep_latest : int, default 5
        Number of most recent backtests to preserve.
    max_total_size_mb : int, default 500
        Maximum aggregate size in megabytes for retained backtests.
    """
    try:
...
def kill_process_on_port(port):
    """
    Terminate any local process currently bound to the specified port.

    Parameters
    ----------
    port : int
        TCP port number to free.
    """
    try:
...
def log_request_info():
    """
    Log incoming request metadata for auditing purposes.
...
def log_response_info(response):
    """
    Log outbound response metadata for auditing purposes.

    Parameters
    ----------
    response : flask.Response
        Response object generated by the view.

    Returns
    -------
    flask.Response
        Same response passed into the function.
    """
...
class BacktestManager:
    """
    Coordinate data downloads and Lean backtest execution.

    Attributes
    ----------
    process : subprocess.Popen or None
        Handle to the active Lean backtest process.
    download_process : subprocess.Popen or None
        Handle to the active data download process.
    status : str
        Lifecycle state string (e.g., ``"idle"``, ``"running_backtest"``).
    """
    def __init__(self):
        self.process = None
    ...
    def _check_data_availability(self, symbols, start_date, end_date, polygon_api_key):
        """
        Verify availability of second-resolution data for the requested symbols.

        Parameters
        ----------
        symbols : list[str]
            Symbol tickers requested for the backtest.
        start_date : str
            Inclusive start date string formatted as ``YYYY-MM-DD``.
        end_date : str
            Inclusive end date string formatted as ``YYYY-MM-DD``.
        polygon_api_key : str or None
            Polygon API key (currently unused but retained for compatibility).

        Returns
        -------
        tuple[list[str], list[str]]
            Tuple of ``(missing_symbols, available_symbols)``.
        """
        import os
    ...
    def _start_lean_execution(self, algorithm_name, config_params):
        """
        Launch a Lean CLI backtest using pre-generated configuration.

        Parameters
        ----------
        algorithm_name : str
            Name of the Lean project to execute.
        config_params : dict or None
            Additional CLI parameters to append to the Lean invocation.
        """
    ...
    def start_backtest(self, algorithm_name="ZacQC", config_params=None):
        """
        Kick off a new backtest in a background thread.

        Parameters
        ----------
        algorithm_name : str, default "ZacQC"
            Name of the Lean project to execute.
        config_params : dict or None
            Additional CLI parameters to append to the Lean invocation.
        ...
        -------
        dict
            Status message describing whether the backtest was accepted.
        """
    ...
    def _run_backtest_async(self, algorithm_name="ZacQC", config_params=None):
        """
        Execute the backtest asynchronously and capture errors.
    ...
    def _execute_backtest_sync(self, algorithm_name="ZacQC", config_params=None):
        """
        Run the full data-prep and Lean execution pipeline.
        ...
        try:
            ...
            if should_download and polygon_api_key:
                ...
                def batch_symbols(symbol_list, batch_size=1):
                    """
                    Yield successive batches of symbols to keep CLI arguments short.
                    ...
                    ------
                    list[str]
                        Subset of symbols sized according to ``batch_size``.
                    """
    ...
    def stop_docker_containers(self):
        """
        Stop Docker containers whose names indicate Lean usage.

        Returns
        -------
        bool
            True when at least one container was stopped, otherwise False.
        """
        try:
    ...
    def stop_backtest(self):
        """
        Terminate active data download or backtest processes.

        Returns
        -------
        bool
            True when a process was stopped, otherwise False.
        """
    ...
    def get_status(self):
        """
        Summarise the lifecycle state and timing metadata for the backtest.

        Returns
        -------
        dict
            Status dictionary including phase, durations, and download progress.
        """
        if self.process:
    ...
    def get_logs(self, last_n=50):
        """
        Return the most recent log entries tracked by the manager.
        ...
        -------
        dict
            Dictionary containing ``logs`` and ``total_count`` keys.
        """
        return {
    ...
    def _monitor_logs(self):
        """
        Consume Lean stdout in a background thread and persist log entries.
    ...
    def _find_latest_results(self):
        """
        Discover the newest backtest results JSON file and cache its path.
...
def start_backtest():
    """
    Trigger the backtest manager to launch a new Lean run.

    Returns
    -------
    flask.Response
        JSON payload describing launch status.
    """
...
def stop_backtest():
    """
    Request cancellation of the active backtest or download job.

    Returns
    -------
    flask.Response
        JSON payload indicating whether processes were stopped.
    """
...
def get_status():
    """
    Report the current status of the backtest manager.

    Returns
    -------
    flask.Response
        JSON payload containing state and timing metadata.
    """
...
def get_logs():
    """
    Fetch recent log entries produced by the backtest manager.

    Returns
    -------
    flask.Response
        JSON payload containing log entries and counts.
    """
...
def get_results():
    """
    Return the most recently generated Lean backtest results payload.

    Returns
    -------
    flask.Response
        JSON response containing results or an error message.
    """
    global latest_results_path
...
def scan_results():
    """
    Force a filesystem scan to locate the most recent results file.

    Returns
    -------
    flask.Response
        JSON payload describing whether a results file was found.
    """
    backtest_manager._find_latest_results()
    global latest_results_path
...
def health_check():
    """
    Provide a heartbeat response for monitoring systems.

    Returns
    -------
    flask.Response
        JSON payload containing status and timestamp.
    """
...
def get_data_download_status():
    """
    Report whether global data downloads are disabled.

    Returns
    -------
    flask.Response
        JSON payload with boolean state and message.
    """
...
def disable_data_download():
    """
    Disable Lean data downloads until re-enabled via the API.

    Returns
    -------
    flask.Response
        JSON payload reflecting the new disabled state.
    """
    global DISABLE_DATA_DOWNLOAD
...
def enable_data_download():
    """
    Re-enable Lean data downloads after a previous disable call.

    Returns
    -------
    flask.Response
        JSON payload reflecting the new enabled state.
    """
    global DISABLE_DATA_DOWNLOAD
...
def get_config():
    """
    Load trading configuration from `parameters.py`.

    Returns
    -------
    flask.Response
        JSON payload containing configuration values or an error.
    """
...
def save_config():
    """
    Persist configuration overrides and validate the updated file.

    Returns
    -------
    flask.Response
        JSON payload confirming success or describing the error.
    """
...
def validate_config():
    """
    Verify that `parameters.py` can be parsed into a configuration dictionary.

    Returns
    -------
    flask.Response
        JSON payload indicating validity and parameter count.
    """
    try:
        config_dict = load_config_strict(CONFIG_FILE_PATH)
...
def index():
    """
    Serve the main HTML entry point for the frontend.

    Returns
    -------
    flask.Response
        HTML response with caching disabled.
    """
...
def config_page():
    """
    Serve the configuration editor single-page app.

    Returns
    -------
    flask.Response
        HTML response for the config editor.
    """
    frontend_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'frontend')
...
def serve_static(filename):
    """
    Serve static frontend assets from disk.

    Parameters
    ----------
    filename : str
        Relative path to the asset within the frontend directory.

    Returns
    -------
    flask.Response
        File response streaming the requested asset.
    """
    frontend_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'frontend')
